...
const serviceLabels: Record<(typeof CLASSIC_OUTBOUND_SERVICE_IDS)[number], string> = {
  "sdr-team": "SDR Team",
  "ae-team": "AE Team",
};
...
function OutboundConfirmationSummary(): React.JSX.Element {
  ...
  const selectedOutboundLabels = selectedServices
    .filter((id): id is (typeof CLASSIC_OUTBOUND_SERVICE_IDS)[number] =>
...
function EventConfirmationSummary(): React.JSX.Element {
  const { state, dispatch } = useCalculator();
...
export default function StepPerformance(): React.JSX.Element {
  ...
  if (isEventOnly) {
    return <EventConfirmationSummary />;
  }
  ...
  return (
    <p className="text-center text-sm text-[var(--color-text-secondary)]">
      No performance configuration for the selected services.
    </p>
  );
}
